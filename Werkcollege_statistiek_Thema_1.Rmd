...
Het is ook handig om je 'working directory' te veranderen in deze map met databestanden. Dit kun je doen via het menu: Session - Set Working Directory - Choose Directory... of met code: `setwd("pathname")`. Let op dat je in je pad geen backslash (\\) moet gebruiken maar of 'forward slashes' (/) of een _dubbele_ backslash. Je kunt er ook voor kiezen om pad te definiëren en te werken met de `file.path()` functie, zoals we in de opgaven hieronder doen.  

Verder maken we gebruik van functies uit de `psych` package. Als je die nog niet hebt geïnstalleerd, doe dat nu. Daarna kunnen we hem laden:
...
### 1.1 Data inlezen
De data zijn opgeslagen in een het bestand `OD250.csv`. We lezen de data in en noemen de data frame d1 (NB: niet vergeten om de naam van de map, hier `mypath` genoemd, te veranderen in de map waar jij het bestand heb opgeslagen). We bekijken de eerste regels met `head`:


```{r}
mypath <- "Werkcollege_statistiek_Thema_1"
d1 <-read.table(file.path(mypath, "OD250.csv"), sep=",", header=TRUE) 
dim(d1)
head(d1)
```
...
```{r}
od.dat <-read.table(file.path(mypath, "ODdatavb.csv"), sep=",", header=TRUE) 
od.dat
```
