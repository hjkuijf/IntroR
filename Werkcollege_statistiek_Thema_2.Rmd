...
* [CSFdat.csv](Werkcollege_statistiek_Thema_2/CSFdat.csv)
* [zalmDNAconcabs.csv](Werkcollege_statistiek_Thema_2/zalmDNAconcabs.csv)


Het is ook handig om je 'working directory' te veranderen in deze map met databestanden. Dit kun je doen via het menu: Session - Set Working Directory - Choose Directory... of met code: `setwd("pathname")`. Let op dat je in je pad geen backslash (\\) moet gebruiken maar of 'forward slashes' (/) of een _dubbele_ backslash.  

Verder maken we gebruik van functies uit de packages `ggplot2` en `investr`. Als je die nog niet hebt geïnstalleerd, doe dat nu. Daarna kunnen we hem laden:
...
### 1.1 Data uit .csv inlezen
We beginnen met het inlezen van het bestand en dat bekijken. (_NB: niet vergeten om de naam van de map, hier `mypath` genoemd, te veranderen in de map waar jij het bestand heb opgeslagen!_) De data staat in een .csv bestand `huisstof.csv`. We lezen de data in en noemen de data frame d1 (NB: niet vergeten om de naam van de map, hier `mypath` genoemd, te veranderen in de map waar jij het bestand heb opgeslagen). We bekijken de eerste regels met `head`:

```{r}
mypath <- "Werkcollege_statistiek_Thema_2"
d1 <- data.frame(read.csv(file.path(mypath, "huisstof.csv"), sep=","))
head(d1)
```
...
```{r, eval=-2}
d2 <- data.frame(read.csv(file.path(mypath, "CSFdata.csv"), sep=","))
head(d2)
```
...
```{r, eval=-2}
d3 <- data.frame(read.csv(file.path(mypath, "zalmDNAconcabs.csv"), sep=","))
d3
```
...
**Vraag 3f. Gebruik de `invest()` functie (tip: gebruik de help functie!) uit de `investr` package om je voorspelling uit 3c te checken, en om predictieinterval rondom die voorspelling te krijgen. Interpreteer dit interval. Voor het type interval mag je de default gebruiken.**  
