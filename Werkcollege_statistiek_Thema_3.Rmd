...
Hiervoor lees je de spirometrie data (`'spirometrie.csv'`) allereerst in R in. Zorg er voor dat je het bestand van BB hebt gehaald en lokaal hebt bewaard.

We beginnen met het inlezen van het bestand en dat bekijken. (_NB: niet vergeten om de naam van de map, hier `mypath` genoemd, te veranderen in de map waar jij het bestand heb opgeslagen!_) De data staat in een .csv bestand `spirometrie.csv`. We lezen de data in en noemen de data frame d (NB: niet vergeten om de naam van de map, hier `mypath` genoemd, te veranderen in de map waar jij het bestand heb opgeslagen).

```{r}
mypath <- "Werkcollege_statistiek_Thema_3"
...
In R is er in de package `BSDA` een z-toets beschikbaar. Wil je de z-waarde uitrekenen in R, dan gaat dit als volgt. Voor de mannen:
